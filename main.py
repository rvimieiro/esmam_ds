...
def stats_results(sg_baseline, _it_init=0, _dbs_list=None, _save_log=False):
    ...
    for file in csv_files:
        ...
        for exp in range(_it_init, 2):  # statistical experiments
            ...
            if sg_baseline=='complement':
                run(file=file, dtypes=dtypes, sg_baseline=sg_baseline, seed=seeds[exp], save_path=save_name,
...
if __name__ == '__main__':
    ...
    # pipeline for satistical execution procedure
    stats_results(sg_baseline='complement', _save_log=True, _it_init=0, _dbs_list=DB_NAMES)
    stats_results(sg_baseline='population', _save_log=True, _it_init=0, _dbs_list=DB_NAMES)
